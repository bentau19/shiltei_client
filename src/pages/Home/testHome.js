...
function ProductComp({ info, view }) {
    return (
      <div className={`rela-inline product-card`} key={info.id} style={{ 'animationDelay': `${info.delay * 0.1}s` }}>
        <div className="rela-block product-pic" style={{ 'background': `url(${info.img}) center no-repeat` }}>
          <div className="product-view-button" onClick={() => view(info.id)}>View</div>
        </div>
        <div className="rela-block product-info">
          <div className="rela-block">
            <p>{info.name}</p>
            <p className="product-artist">{info.artist}</p>
          </div>
          <div className="vert-center product-cost">${info.cost}</div>
        </div>
      </div>
    );
  }
const App2=({menuOpen})=> {
    ...
    const [products, setProducts] = useState([
        ...
        },
        {
            ...
            catg: "Catg B",
            img:
                "https://i.scdn.co/image/fdce21f4a373a44f831f71f70cbbd02360b676c9",
        ...
        },
        {
            ...
            catg: "Catg A",
            img:
                "https://i.scdn.co/image/ed32b0404f7077962bfe61f1b176a8c868bc2cf3",
            date: "April 7, 2015"
        },
        {
            id: 13,
            ...
            catg: "Catg A",
            img:
                "https://i.scdn.co/image/fea11053422e15bacd0affe4527d2afc4b9b2d06",
        ...
        },
        {
            ...
            catg: "Catg B",
            img:
                "https://i.scdn.co/image/c3a72121de44fb85f91c7d460af800cf4bb54e0a",
            date: "April 29, 2016"
        },
        {
            id: 16,
            ...
            catg: "Catg E",
            img:
                "https://i.scdn.co/image/6268950a5d1451fb33088c9237b26083785abf2a",
            date: "April 20, 2012"
        },
        {
            id: 17,
    ...
    const catg = ["All", "Catg A", "Catg B", "Catg C", "Catg D", "Catg E"];
    const [currentCatg, setCurrentCatg] = useState("All");
    ...
    const updateFilteredProducts = () => {
      const filtered = products.filter(
        (el) => el.catg === currentCatg || currentCatg === "All"
      );
    ...
    const updateViewedProduct = () => {
      const viewed = products.filter((el) => el.id === currentViewedProduct)[0];
      setViewedProduct(viewed);
    };
  
    const viewProduct = (id) => {
      setCurrentViewedProduct(id);
      updateViewedProduct();
  ...
  <div style={{height:"80px"}}></div>
    <div className="rela-block page-section top-section">
      <div className="gutter-container">
        <div className="rela-block section-nav">
          <h2 className="left">New Item Showcase</h2>
        </div>
        <div className="rela-block new-item-container">
          <div className={`vert-center move-arrow left-arrow ${newItemPos <= 0 ? 'disabled' : ''}`} onClick={() => updateNewItemPos(-1)}>&lt;</div>
          <div className={`vert-center move-arrow right-arrow ${newItemPos >= (newItems.length - 1) ? 'disabled' : ''}`} onClick={() => updateNewItemPos(1)}>&gt;</div>
          <div className="inner-moving-container" style={{ 'left': `${(newItemPos * -320) + 1}px` }}>
            {newItems.map((ni, index) => (
              <div key={ni.id} className="rela-inline new-item" style={{ 'background': `url('${ni.img}') center no-repeat`, 'animation-delay': `${index * 0.1}s` }}>
                <div className="new-item-info">
                  <p className="abs-center">{ni.name}<br />{ni.artist}</p>
                </div>
                <div className="product-view-button" onClick={() => viewProduct(ni.id)}>View</div>
              </div>
            ))}
          </div>
        </div>
    ...
    <div className="rela-block page-section grey product-section">
      <div className="rela-block gutter-container">
        <div className="rela-block section-nav">
          <h2 className="left">All Products<span>{currentCatg !== 'All' ? `/${currentCatg}` : ''}</span></h2>
          <div className="right category-select">
            {catg.map((c) => (
              <div key={c} className={`rela-inline category ${currentCatg === c ? 'active' : ''}`} onClick={() => { setCurrentCatg(c); updateFilteredProducts(); }}>{c}</div>
            ))}
          </div>
        </div>
        <div className="rela-block product-item-container">
          {displayedProducts.map((item, index) => (
            <ProductComp key={item.id} info={item} view={viewProduct} />
          ))}
        </div>
  ...
  </div> <div className={`product-view-container ${productViewOpen ? 'active' : ''}`}>
        <div className="rela-block gutter-container">
          <div className="rela-block section-nav">
          ...
          <div className="rela-block pv-container">
            <div className="rela-block pv-pic" style={{ 'background': `url('${viewedProduct.img}') center no-repeat` }} />
          </div>
          <div className="rela-block pv-container">
            <div className="rela-block pv-info">
              <h2 className="rela-block">{viewedProduct.name}</h2>
              <h3 className="rela-block">{viewedProduct.artist}</h3>
              ...
              <div className="rela-block info-row">
                ...
                <div className="rela-block text">{viewedProduct.catg || 'No Category'}</div>
              </div>
              <div className="rela-block info-row">
